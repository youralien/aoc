...
(defun checksum (sheet)
  (reduce #'+
    (mapcar #'(lambda (row) (- (apply #'max row) (apply #'min row)))
            sheet)))

(defun evenly-divisible-values (sheet)
  (reduce #'+
    (mapcar
      #'(lambda (row)
          (dolist (pair (every-pair row))
            (let ((even-quotient (evenly-divisible (car pair) (cdr pair))))
              (when even-quotient
                  (return even-quotient)))))
      sheet)))

; this function is provided in python using itertools.pairs?
(defun every-pair (row)
  (cond
    ((null (cdr row)) nil)
    (t (append (mapcan #'(lambda (x) (acons (car row) x nil))
                       (cdr row))
               (every-pair (cdr row))))))

; this function is modulus operator in python...
(defun evenly-divisible (x y)
  (multiple-value-bind (quotient remainder)
    (if (> x y) (floor x y) (floor y x))
    (if (zerop remainder)
        quotient
      nil)))

(print (every-pair (car *spreadsheet*)))
    
(print (evenly-divisible 8 2))
(print (evenly-divisible 100 8))
(print (checksum *spreadsheet*))
(print (evenly-divisible-values *spreadsheet*))
